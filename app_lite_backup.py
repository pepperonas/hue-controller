...
def start_strobe():
    ...
    data = request.get_json()
    target_type = data.get('type', 'light')  # 'light', 'group', oder 'all'
        ...
        while time.time() - start_time < duration and effect_id in running_effects:
            if target_type == 'all':
                ...
                for light_id in lights.keys():
                    hue_request(f'lights/{light_id}/state', 'PUT', {'on': True, 'bri': 254})
                time.sleep(interval / 2)
                
                for light_id in lights.keys():
                    hue_request(f'lights/{light_id}/state', 'PUT', {'on': False})
                time.sleep(interval / 2)
            else:
...
def start_advanced_effect(effect_type):
    ...
    if effect_type == 'wave':
        return start_wave_effect(effect_id, target_type, duration, speed)
    elif effect_type == 'pulse':
        return start_pulse_effect(effect_id, target_type, duration, speed)
    elif effect_type == 'rainbow':
        return start_rainbow_effect(effect_id, target_type, duration, speed)
    elif effect_type == 'fire':
        return start_fire_effect(effect_id, target_type, duration, speed)
    elif effect_type == 'sunset':
        return start_sunset_effect(effect_id, target_type, duration, speed)
    elif effect_type == 'lightning':
        return start_lightning_effect(effect_id, target_type, duration, speed)
    else:
        return jsonify({"error": "Unknown effect type"})
